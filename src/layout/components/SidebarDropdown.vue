<template>
    <li :class="{ 'nav-item': true, 'menu-open': open }">
        <a :class="{ 'nav-link': true, active: active }" @click.prevent="toggle" href="#">
        ...
        <transition @before-enter="beforeEnter" @enter="enter" @leave="leave" name="aside-dropdown">
            <ul class="nav nav-treeview" ref="dropdown" v-show="open">
                <slot/>
            </ul>
        </transition>
    </li>
